...
def equals(param1,param2,error):
...
def PIDcontrollinear(current_coord,dest_coord,motorpin):
    ...
    error_x = equals(x_dest_coord,x_current_coord,linear_error) # calculates if error in x direction
    error_y = equals(y_dest_coord,y_current_coord,linear_error) # calculates if error in y direction
...
def PIDcontrolrotate(current_coord_dict,dest_coord_dict,motorpin):
    ...
    error = equals(dest_coord,current_coord,rotate_error)
...
def parse_direction(data):
    ...
    anglediff = equals(float(data["target"]["theta"]),float(data["pose"]["theta"]),rotate_error) # calculate difference of angle of dst and src
    if anglediff == 0:
        # only for forward and reverse
        if data["pose"]["theta"] == 0.0:
            # if pointing towards +ve x axis
            xdiff = equals(float(data["target"]["x"]),float(data["pose"]["x"]),linear_error) # dst - src (required difference is +ve)
            if xdiff > 0.0:
                return "fw"
            elif xdiff < 0.0:
                return "rv"
            ...
            ydiff = equals(float(data["target"]["y"]),float(data["pose"]["y"]),linear_error) # dst - src (required difference is +ve)
            if ydiff > 0.0:
                return "fw"
            elif ydiff < 0.0:
                return "rv"
